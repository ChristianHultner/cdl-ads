...
/**
 * match-b0-hc.mjs
 * Reads a b0-titles JSON file + catalog from /api/v1/books/public/list.
 * Proposes B0 Kindle → HC pairings.
 *
 * Usage:
 *   node --env-file=.env.local scripts/match-b0-hc.mjs
 *     [--input <path>]   default: artifacts/b0-titles-head.json
 *     [--execute]        write PROPOSED rows to DB (idempotent, never overwrites CONFIRMED/REJECTED)
 *
 * Dry-run default: prints band distribution + sample pairs, then exits.
 * --execute: upserts PROPOSED rows into b0_hc_candidates.
 */
...
const inputPath = cliValues.input
  ? (cliValues.input.startsWith('/') ? cliValues.input : join(process.cwd(), cliValues.input))
  : join(ARTIFACTS_DIR, 'b0-titles-head.json');
const execute = cliValues.execute === true;
...
// ---------------------------------------------------------------------------
const EDITION_NOISE = [
  /versi[oó]n kindle/gi,
  /\(spanish edition\)/gi,
  /\(english edition\)/gi,
  /ebook kindle/gi,
  /edici[oó]n kindle/gi,
  /\bkindle\b/gi,
  /\bebook\b/gi,
  /\bedici[oó]n\b/gi,
  /\bboard book\b/gi,
...
const ES_STOPS = new Set(['el','la','los','las','un','una','unos','unas','de','del',
  'en','y','que','es','con','por','para','al','se','le','su','pero','como',
...
async function main() {
  ...
  // Load b0 titles from --input (or default head file)
  console.log(`Loading titles from: ${inputPath}`);
  const rawTitles = JSON.parse(await readFile(inputPath, 'utf8'));
  console.log(`Loaded ${rawTitles.length} entries.`);

  // Map asin → title (null if unavailable / non-ok)
  const asinTitle = {};
  ...
  for (const [b0asin, rawTitle] of Object.entries(asinTitle)) {
    ...
    // Language mismatch: halve confidence
  ...
  // Print 15 samples (proposed first, then no-match)
  console.log('\n=== 15 SAMPLES ===');
  const samples = [
    ...proposed.slice(0, 10),
    ...noMatch.slice(0,  5),
  ...
  for (const p of proposals) {
    ...
    `, [p.b0_asin, p.amazon_title, p.hc_isbn13, p.hc_title,
        p.confidence, p.method, p.matched_via, p.status]);
