...
// ---------------------------------------------------------------------------
// Window: default (today − 14 days)..yesterday; overridden by --start/--end.
// NOTE: Amazon caps report ranges (~31 days for spCampaigns / spSearchTerm).
// Do NOT chunk internally — if the range is rejected, that is the caller's
// problem to split.
// ---------------------------------------------------------------------------
...
if (values.start || values.end) {
  ...
  if (!startDate || !ISO_DATE.test(startDate))
    throw new Error(`--start must be YYYY-MM-DD, got: ${startDate}`);
  if (!endDate || !ISO_DATE.test(endDate))
    throw new Error(`--end must be YYYY-MM-DD, got: ${endDate}`);
  if (startDate > endDate)
    throw new Error(`--start (${startDate}) must be <= --end (${endDate})`);
  if (endDate >= todayStr)
    throw new Error(`--end (${endDate}) must be in the past (today is ${todayStr})`);
} else {
