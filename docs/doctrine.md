...
**Two-step handoff.** `request-reports` runs at 00:30 UTC. It requests `spCampaigns`, `spSearchTerm`, and `spAdvertisedProduct`, using the nightly's exact SPONSORED_PRODUCTS / DAILY / GZIP_JSON configurations and a Madrid calendar window of today minus seven days through yesterday. Each returned report ID is stored in `amazon_report_requests` as PENDING; this step never polls. An existing PENDING, COMPLETED, or INGESTED request for the same profile/type/window is skipped. A per-step advisory lock serializes overlapping cron/dashboard invocations; a duplicate-report HTTP 425 can recover the original ID if POST succeeded before a failed INSERT.

`ingest-reports` runs at 01:30, 02:30, 03:30, 05:30, and 07:30 UTC (one cron expression, `30 1,2,3,5,7 * * *`). It selects eligible PENDING/COMPLETED requests oldest first, checks each once, leaves unfinished work for later invocations, marks Amazon failures FAILED with an error, and expires PENDING requests older than three days as EXPIRED. COMPLETED is persisted before downloading so a transient download/ingest failure remains retryable. Every completed report gets a fresh signed URL; the S3 download has no Amazon auth headers. No signed URLs or tokens are persisted or logged.

**Scope.** Both steps are restricted to `amazon_profiles.is_active=true` AND the constant profile allowlist: `2263723137827296` (ES), `139446882235960` (US), `395707988492653`, `350599867165328`, `1711934819800765`. All other IDs are excluded; FR/IT/CA2 are excluded by design. Credentials come from the profile's `amazon_credentials.env_var_name`; LWA minting is per profile, with the nightly's 401 re-mint and one retry. Sensitive production variables pulling blank locally is expected; never replace them to make a local sync run.

**Landing and limits.** `spCampaigns` lands in `amazon_campaign_daily`, `spSearchTerm` in `amazon_search_term_daily`, and `spAdvertisedProduct` in `amazon_advertised_product_daily`. Column mappings and ON CONFLICT clauses match the nightly. All three use batches of 500 (campaign-daily replaces its old single-row loop). One transaction per report includes a locked status recheck, every upsert, and the INGESTED/completed_at update. Failure rolls back all landing and leaves the report retryable. Whole-buffer download, `gunzipSync`, and JSON parsing are intentionally retained; reports are processed serially and buffers released. The requested Amazon-only function configuration is memory 2048 MB and duration 300 s. Vercel deployment acceptance/effective memory remains unverified until deployment; with Fluid compute, memory is dashboard-controlled and the JSON memory setting is not applied ([Vercel memory documentation](https://vercel.com/docs/functions/configuring-functions/memory)). No project-wide setting or Google function is changed here.

Both steps check the 240-second soft wall cap between reports/requests and record `PARTIAL n/total, continues next run`. Pending rows or known windows are the durable resume mechanism, not an in-memory cursor. An in-progress report completes atomically or rolls back at the 270-second cleanup deadline; HTTP timeouts cover response bodies, and SQL statement timeouts are bounded. Errors remain `ok=false`, including partial runs containing errors. Each invocation owns and closes its Pool. `amazon_sync_log` records INSERT-start (ok NULL), then UPDATE-finish (ok, rows_reported, detail); the entire logging/cleanup lifecycle is caught so `runStep` never rejects, even when logging itself fails. A database outage can prevent a finish record and is returned as failure, never success.

**No cutover.** The mini's launchd schedule and every `scripts/*.mjs` remain unchanged and keep running in parallel, including nightly report landing, campaign structure sync, daily rollup, stale-recommendation sweep, outcome stamping, and existing weekly/watchdog work. Vercel's `structure`, `rollup`, `sweep`, and `stamp` are explicitly NOT_YET_IMPLEMENTED: they log/return `ok=false`, detail `not implemented`, and have no cron entries. Do not cut over until seven consecutive nights show identical independently captured landing row counts from both paths, per allowlisted active profile and report type over the same seven-day window (the mini defaults to fourteen days, so raw whole-window totals are not comparable). A shared table's final count alone is not proof that both paths succeeded. No push or live invocation is part of this frame; Christian triggers the first live run from the Vercel dashboard after a later authorized push.
...
**What Moved retirement (dashboard overhaul frame 3, 2026-08-30):** The What Moved band and movers tables are no longer rendered on the dashboard; movers data remains available through the scorecard.
...
**Channel GP (display) (dashboard overhaul frame 2, 2026-08-30; asynchronous truth-layer refresh, 2026-09-09):** On a unit-basis market card, trailing-quarter channel GP = `SUM(vendor_history.units) × amazon_profiles.gp_per_order − SUM(console_history.spend)` across the latest three consecutive months present in **both** `vendor_history` and `console_history`; console spend is the all-ad-types monthly export. The block window is therefore the latest common consecutive quarter, not necessarily the latest vendor quarter, and its label must state that window. A newer unmatched month in either layer must not suppress the block. When the layers end on different months, the block shows a small muted `vendor through <Mon>, console through <Mon>` line so staleness is explicit. The block renders only when `gp_per_order` is ruled and a three-month common consecutive window exists. Units/month = three-month vendor units ÷ 3, and attribution share = same-window console orders ÷ vendor units. `vendor_history` and `console_history` are queried separately and passed as separate props; they meet only in `ChannelBlock`, never in SQL or page shaping, and neither is joined to `daily_rollup` or `experiments`. The vendor sell-in caveat remains in force: purchasing is batched and lagged, and channel GP is therefore a quarterly-grain display metric only, never an engine, grading, generation, push, or watchdog input.
...
**Vendor sell-in truth layer (`vendor_history`, 2026-08-30):** Amazon Vendor invoice history is a second display-only truth layer beside `console_history`. It records Amazon's replenishment purchasing (sell-in), not customer sell-through; purchasing is batched and lagged, so monthly rows may be read only at quarterly-or-coarser grain for trend judgments. `vendor_history` is **never joined to `daily_rollup`, `console_history`, or any other table** and is **never read by generation, grading, or watchdog rules**. The Long-term chart renders rolling-12 vendor revenue as a dashed overlay on the native-currency money axis and rolling-12 vendor units as a dashed line in a separate sell-in-vs-attributed units panel. The chart x-axis is the union of valid rolling-12 endpoint months present across the separately shaped console and vendor layers; each series has no point where its own layer lacks that endpoint, so one layer may visibly extend past the other. The two truth layers meet only as separately shaped component props, never in a database query. The units panel face carries the quarterly-grain caveat and is omitted when a market has no vendor rows.

**Rolling-12 window rule:** A rolling-12 point at month M = sum of months M−11 through M (inclusive, 12 months). A console point is plotted only when all 12 consecutive months are present in `console_history`; a vendor point is plotted only when all 12 consecutive months are present in `vendor_history`. Partial windows are never plotted. The x-axis is the union of the valid endpoint months from both layers, and an absent month in one layer does not remove the other layer's point. A market is excluded only when neither layer has a valid rolling-12 point.
